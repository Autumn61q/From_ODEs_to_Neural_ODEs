...
# --- 自定义二阶 HNN，使用 Midpoint 积分器 ---
class SecondOrderHNN(ln.nn.Algorithm):
    def __init__(self, layers, activation='tanh', initializer='orthogonal'):
        super(SecondOrderHNN, self).__init__()
        self.H_size = layers
    ...
    def criterion(self, x, y):
        ...
        def get_v(z):
            # 使用 learner.utils.grad with create_graph=True for gradient flow
            dH = grad(self.ms['H'](z), z, create_graph=True)
            return dH @ self.J

        # Midpoint method (RK2): k1, k2
        k1 = get_v(x_in)
    ...
    def predict(self, x0, h, steps=1, keepinitx=False, returnnp=False):
        ...
        solver = Midpoint(self.ms['H'], self.J, N=N)
...
def run_diff_h_experiment():
    ...
    for h in hs:
        ...
        for run in range(num_runs):
            ...
            # 使用自定义的 2 阶 Midpoint HNN
            net = SecondOrderHNN(H_size, activation='tanh')
