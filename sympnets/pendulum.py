...
def plot(data, net):
    ...
    if isinstance(net, ln.nn.HNN):
        flow_true = data.solver.flow(data.X_test_np[0][0], data.h, steps)
        flow_pred = net.predict(data.X_test[0][0], data.h, steps, keepinitx=True, returnnp=True)
        p, q = data.X_train_np[0][:, 0], data.X_train_np[0][:, 1]
    else:
        ...
        p, q = data.X_train_np[:, 0], data.X_train_np[:, 1]
    
    plt.plot(flow_true[:, 0], flow_true[:, 1], color='b', label='Ground truth', zorder=1)
    plt.plot(flow_pred[:, 0], flow_pred[:, 1], color='r', label='Predicted flow', alpha=0.3, zorder=3, linewidth=2)
    plt.scatter(p, q, color='b', label='Learned data', zorder=2, alpha=0.7, s=10)
    plt.legend()
