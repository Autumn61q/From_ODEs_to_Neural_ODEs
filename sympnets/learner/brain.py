...
class Brain:
    ...
    def output(self, data, best_model, loss_history, info, path, **kwargs):
        if path is None:
            path = self.__generate_output_path()
        ...
        if loss_history:
            self.__save_loss_history(path)
        # Save config
        config = self.__generate_config(info)
    ...
    def __generate_output_path(self):
        """Generate detailed output path based on network type and parameters."""
        net_type = self.net.__class__.__name__
        timestamp = str_current_time()
        
        # Extract network parameters for detailed naming
        params = []
        if hasattr(self.net, 'layers'):
            params.append(f'layers{self.net.layers}')
        if hasattr(self.net, 'sublayers'):
            params.append(f'sublayers{self.net.sublayers}')
        if hasattr(self.net, 'width'):
            params.append(f'width{self.net.width}')
        if hasattr(self.net, 'activation'):
            params.append(f'{self.net.activation}')
        
        param_str = '_'.join(params) if params else 'default'
        path = f'./outputs/{net_type}_{param_str}_{timestamp}'
        return path
    
    def __save_loss_history(self, path):
        """Save loss history with detailed formatting."""
        if len(self.loss_history_list) == 1:
            loss_data = self.loss_history
            header = 'Iteration\tTrain_Loss\tTest_Loss'
            np.savetxt(path + '/loss.txt', loss_data, header=header, fmt='%d\t%.6e\t%.6e', comments='')
        else:
            for i in range(len(self.loss_history_list)):
                loss_data = self.loss_history_list[i]
                header = 'Iteration\tTrain_Loss\tTest_Loss'
                np.savetxt(path + '/loss{}.txt'.format(i + 1), loss_data, header=header, fmt='%d\t%.6e\t%.6e', comments='')
    
    def __generate_config(self, info):
        """Generate configuration dictionary."""
        config = {
            'timestamp': str_current_time(),
            'experiment': {
                'problem': info.get('problem', 'unknown') if info else 'unknown',
                'network': info.get('network', 'unknown') if info else 'unknown',
            },
            'model': {
                'type': self.net.__class__.__name__,
                'dim': self.net.dim if hasattr(self.net, 'dim') else None,
            },
            'training': {
            ...
            'data': {
                'train_samples': len(self.data.X_train) if hasattr(self.data.X_train, '__len__') else 'unknown',
                'test_samples': len(self.data.X_test) if hasattr(self.data.X_test, '__len__') else 'unknown',
            },
            'device': self.device,
            'dtype': self.dtype,
        }
        ...
        if hasattr(self.net, 'layers'):
            config['model']['layers'] = self.net.layers
        if hasattr(self.net, 'sublayers'):
            config['model']['sublayers'] = self.net.sublayers
        if hasattr(self.net, 'width'):
            config['model']['width'] = self.net.width
        if hasattr(self.net, 'activation'):
            config['model']['activation'] = self.net.activation
        if hasattr(self.net, 'latent_dim'):
            config['model']['latent_dim'] = self.net.latent_dim
        
        # Add custom info if provided
        if info is not None:
            config['custom_info'] = info
